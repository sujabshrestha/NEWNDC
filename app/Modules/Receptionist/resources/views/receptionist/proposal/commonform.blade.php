            ...
            <select name="bank_id" required class="form-control" id="bank">
                ...
                @if (isset($banks))
                    @foreach ($banks as $bank)
                        <option @if ( (isset($proposal) && $proposal->bank_id == $bank->id)  || old('bank_id') == $bank->id) selected @endif
            ...
            <select name="branch_id" required class="form-control text-capitalize" id="">
                @if (isset($branches))
                    @foreach ($branches as $branch)
                        <option @if ( (isset($proposal) && $proposal->branch_id == $branch->id) || old('branch_id') == $branch->id) selected @endif
    ...
    <div class="col-md-4">
        <div class="form-group">
            ...
            <input type="text" required class="form-control" id="banker_name"
                value="{{ $proposal->banker_name ??  old('banker_name') }}" placeholder="Enter Banker Name" name="banker_name" required>
                @error('banker_name')
    ...
    <div class="col-md-4">
        <div class="form-group">
            ...
            <select class="form-control" required id="status" name="status">
                ...
                <option @if ($proposal->status == 'Pending')
                    selected
                @endif  value="pending">Pending
                </option>
                <option @if ($proposal->status == 'Completed')
                    selected
                @endif value="completed">Completed
                </option>
            </select>
        </div>
    </div>
