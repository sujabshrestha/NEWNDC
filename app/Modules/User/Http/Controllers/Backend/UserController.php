        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                ->addColumn('action', function ($row) {
                    $actionBtn = '<a href="javascript:void(0)" id="'. route('backend.user.edit',$row->id) .'" data-id=' . $row->id . ' class="editUser btn btn-info btn-sm" title="Edit"><i class="fas fa-edit"></i></a>
                                <a href="javascript:void(0)" id="'. route('backend.user.destroy',$row->id) .'" data-id='.$row->id.' class="delete btn btn-danger btn-sm" title="Delete"><i class="fas fa-trash-alt"></i></a>';
                    return $actionBtn;
                })
                ->rawColumns(['action'])
                ->make(true);
